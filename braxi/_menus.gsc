...
onMenuResponse()
{
	...
	for(;;)
	{
		...
		if(menu == game["menu_characters"])
		{
			if(response == "open_characters")
			{
				self hype\_customization::onresponse("characters", "open_characters");
				continue;
			} else if(response == "open_weapons")
			{
				self hype\_customization::onresponse("weapons", "open_weapons");
				continue;
			}
		}
		...
		{
			self hype\_customization::onresponse("characters", response);
		}		
		else if( menu == "dr_weapons" )
		{
			self hype\_customization::onresponse("weapons", response);
		}
		...
		else if( menu == game["menu_team"] )
		{
			...
			case "prestige":
				self closeMenu();
				self closeInGameMenu();
				if(self braxi\_rank::canPrestige()) {
					self braxi\_rank::enterPrestige();
					self iprintlnbold("You entered ^5Prestige "+self.pers["prestige"]+"^7!");
					iprintln(self.name+" entered ^5Prestige "+self.pers["prestige"]+"^7!");
				} else {
					if(self braxi\_rank::getPrestigeLevel() >= level.maxPrestige)
					self iprintlnbold("^1You are already max prestige!");
				}
			}
		}
