...
init()
{
	...
	registerScoreInfo( "givexp", 30000);
...
canPrestige()
{
	...
	if(self.pers["rank"] < level.maxRank)
		return false;
	if(self.pers["rankxp"] < getRankInfoMaxXP(level.maxRank))
		return false;
	return true;
}
...
enterPrestige()
{
	// safeguards, logic will be checked in menus, ensure it is here aswell.
...
unlockCharacter()
{
	...
	{
		if( self.pers["rank"] == level.characterInfo[i]["rank"] && self getPrestigeLevel() >= level.characterInfo[i]["prestige"] )
		{
			notifyData = spawnStruct();
...
isCharacterUnlocked( num )
{
	if( num >= level.characterInfo.size || num <= -1)
		return false;
	iprintln("Rank check: " + self.pers["rank"] + " >= " + level.characterInfo[num]["rank"]);
	iprintln("Prestige check: " + self getPrestigeLevel() + " >= " + level.characterInfo[num]["prestige"]);
	if( self.pers["rank"] >= level.characterInfo[num]["rank"] && self getPrestigeLevel() >= level.characterInfo[num]["prestige"] )
		return true;
	return false;
}
...
unlockItem()
{
	...
	{
		if( self.pers["rank"] == level.itemInfo[i]["rank"] && self getPrestigeLevel() >= level.itemInfo[i]["prestige"] )
		{
		notifyData = spawnStruct();
...
isItemUnlocked( num )
{
	if( num > level.numItems || num <= -1)
		return false;
	if( self.pers["rank"] >= level.itemInfo[num]["rank"] && self getPrestigeLevel() >= level.itemInfo[num]["prestige"] )
		return true;
	return false;
}
