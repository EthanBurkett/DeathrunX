...
main()
{
	...
	thread hype\_customization::init();
	buildSprayInfo();
	buildAbilityInfo();
	bestMapScores();
...
playerConnect() // Called when player is connecting to server
{
	...
	if( !isDefined( self.pers["team"] ) )
	{
		...
		self.pers["prestige"] = self braxi\_rank::getPrestigeLevel();
...
endRound( reasonText, team )
{
	...
	{
		players[i] setClientDvars( "show_hud", "false" );
	}
...
antiAFK()
{
	...
	if( !level.dvar["afk"] || self.pers["team"] == "axis" || self.pers["inCustomization"] )
		return;
